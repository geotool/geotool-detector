...
var debug = require('./debug.js')('geotool:detector');
...
var Detector = function Detector(params) {
	...
	var geoCoders = {
		googlemaps: function(lat, lng, callback) {
		...
		openstreetmap: function(lat, lng, callback) {
			if (jQuery) {
				jQuery.ajax({
					...
					debug && debug("Request Failed: " + textStatus + ", " + error);
				});
			} else {
				callback && callback({ error: true, msg: 'jQuery library is not available' });
			}
		}
	}

	var geolocationDetector = function() {
		var detector = new $.Deferred();

		if (true || navigator.geolocation) {
			navigator.geolocation.getCurrentPosition(function success(position) {
				var lat = position.coords.latitude;
				var lng = position.coords.longitude;
				geoCoders['openstreetmap'](lat, lng, function(error, data) {
					if (error) {
						detector.reject(error);
						return;
					}
					detector.resolve(data);
				});
			}, function error() {
	...
	this.detect = function(config, callback) {
		...
		geolocationDetector().done(function(result) {
			...
			geoipDetector().done(function(result) {
				callback(null, result);
			}).fail(function(error) {
				callback(error);
			});
