...
global.geotool.Detector = global.geotool.Detector || require('./lib/geotool-detector');
module.exports = global.geotool.Detector;
}).call(this,typeof self !== "undefined" ? self : typeof window !== "undefined" ? window : {})
},{"./lib/geotool-detector":2}],2:[function(require,module,exports){
'use strict';
...
var Detector = function Detector(params) {
	...
	geoipCheckers.forEach(function(geoipChecker) {
		...
		geoipHandlers.push(function(callback) {
			...
			if (typeof(geoipChecker.headers) === 'object') {
				reqOpts.headers = reqOpts.headers || {};
				Object.keys(geoipChecker.headers).forEach(function(headerKey) {
					reqOpts.headers[headerKey] = geoipChecker.headers[headerKey];
				});
			}
			axios.get(geoipChecker.url, reqOpts).then(function (response) {
	...
	this.detect = function(config, callback) {
		...
		var myPromise = when.promise(function(resolve, reject) {
			...
			var geolocationPromise = when.reject({});
			if (params.isGeoLocationDetectorEnabled !== false) {
				geolocationPromise = geolocationDetector();
			}
			geolocationPromise.then(function(result) {
